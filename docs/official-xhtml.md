# pybunko.official（旧 pyaozora）

青空文庫の**注記付きテキスト**を、公式サイトと同じ**XHTML**に変換する。

```python
from pybunko import to_official_html, to_official_bytes
...
コードは AGPL-3.0-or-later（[docs/LICENSING.md](LICENSING.md)）。青空文庫の収録ファイル・図書カードメタデータ（CC BY 4.0）の利用は
